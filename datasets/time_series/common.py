...
def preprocess_data(times, X, y, final_index, append_times, append_intensity):
    ...
    train_coeffs = spline.natural_cubic_spline_coeffs(times, train_X)
    val_coeffs = spline.natural_cubic_spline_coeffs(times, val_X)
    test_coeffs = spline.natural_cubic_spline_coeffs(times, test_X)
...
def wrap_data(times, train_coeffs, val_coeffs, test_coeffs, train_y, val_y, test_y, train_final_index, val_final_index,
              test_final_index, device, batch_size, test_batch_size, num_workers=4):
    ...
    val_dataloader = dataloader(val_dataset, batch_size=test_batch_size, num_workers=num_workers)
